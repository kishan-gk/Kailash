...
export function FogBandMarquee() {
  ...
  // Use API marquee data if available, else fall back to static clients list
  const apiMarquee = portfolio?.marquee;
  const baseClients = apiMarquee && apiMarquee.length > 0
    ? apiMarquee.map(m => ({ name: m.name, logo: m.logo }))
    : clients;
  // Duplicate for infinite scroll (8 sets)
  const filledClients = [
    ...baseClients, ...baseClients, ...baseClients, ...baseClients,
    ...baseClients, ...baseClients, ...baseClients, ...baseClients,
  ];

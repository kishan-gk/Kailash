...
function Frame() {
  ...
  const apiAbout = portfolio?.about;
  const heading = apiAbout?.heading ?? 'Turning ideas into meaningful digital';
  const bio = apiAbout?.bio ?? "I'm a UI/UX Designer focused on creating intuitive, scalable digital experiences.";
  ...
  return (
    ...
    >
      ...
      >
        <p className="leading-[1.35] whitespace-pre-wrap">{heading} experiences.</p>
      </motion.div>
...
function Frame1() {
  const portfolio = usePortfolioData();
  const apiAbout = portfolio?.about;
  const yearsExp = apiAbout?.years_experience ? String(apiAbout.years_experience) : '2';
  const projectsCount = apiAbout?.projects_count ? String(apiAbout.projects_count) : '30';
  const education = apiAbout?.education ?? 'BCA';

  return (
    <StatsCard delay={animation.delay.slow}>
      <StatItem number={yearsExp} suffix="+" label="Experience" />
      <StatItem number={projectsCount} suffix="+" label="Projects" />
      <StatItem number={education} label="Education" />
    </StatsCard>
  );
}
...
function ContactEmailText() {
  const portfolio = usePortfolioData();
  const email = portfolio?.about?.email ?? 'kailash.kr0508@gmail.com';
  return <p className="leading-[1.5]">{email}</p>;
}

function ContactPhoneText() {
  const portfolio = usePortfolioData();
  const phone = portfolio?.about?.phone ?? '+91 62994 17052';
  return <p className="leading-[1.5]">{phone}</p>;
}
...
function LinkedinLink() {
  const portfolio = usePortfolioData();
  const url = portfolio?.about?.linkedin_url ?? '#';
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="overflow-clip relative shrink-0 size-[40px] transition-transform hover:scale-110 cursor-pointer">
      <Linkedin />
    </a>
  );
}
...
function DribbbleLink() {
  const portfolio = usePortfolioData();
  const url = portfolio?.about?.dribbble_url ?? '#';
  return (
    <a href={url ?? '#'} target="_blank" rel="noopener noreferrer" className="overflow-clip relative shrink-0 size-[40px] transition-transform hover:scale-110 cursor-pointer">
      <Dribbble />
    </a>
...
function BehanceLink() {
  const portfolio = usePortfolioData();
  const url = portfolio?.about?.behance_url ?? '#';
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="overflow-clip relative shrink-0 size-[40px] transition-transform hover:scale-110 cursor-pointer">
      <Behance />
    </a>

...
export function Experience() {
  ...
  const apiExperience = portfolio?.experience ?? [];

  // Logo mapping for known companies (since API may not have logo URLs)
  const logoMap: Record<string, any> = {
    'BrainyDX': imgBrainydx,
    'Digital Innovations': imgDigital,
    'Rablo.in': imgRablo,
  };

  const displayCompanies = apiExperience.length > 0
    ? apiExperience.map(company => ({
        id: String(company.id),
        name: company.name,
        logoType: 'image' as const,
        logoSrc: company.logo ?? logoMap[company.name] ?? imgDigital,

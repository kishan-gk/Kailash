...
export function Expertise() {
  ...
  const apiSkills = portfolio?.skills ?? [];

  // Map API skills to expertise items with fallback illustrations
  const illustrationMap: Record<string, any> = {
    'UI Design': illustration,
    'UX Design': uxIllustration,
    'Flow Diagrams': flowIllustration,
  };

  const displayItems = apiSkills.length > 0
    ? apiSkills.map(s => ({
        title: s.title,
        description: s.description,
        pointers: s.tags,
        illustration: illustrationMap[s.title] ?? illustration,
      }))
    : expertiseItems;
